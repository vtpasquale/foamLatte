classdef SparseTriplet < handle
    ...
    methods
        ...
        function obj = addVector(obj,V,gDof)
            % Adds vector V with global DOF gDof to SparseTriplet
            
            % Number management
            [lengthV,widthV] = size(V);
            if widthV~=1; error('Use addMatrix for widthV>1.'); end
            
            nTripletsOld = obj.nTriplets;
            nTripletsNew = nTripletsOld + lengthV;
            obj.nTriplets = nTripletsNew;
            while nTripletsNew > obj.paddedLength
                obj = padVectors(obj);
            end
            ...
            index = nTripletsOld+1:nTripletsNew;
            obj.i(index) = gDof;
            obj.j(index) = ones(lengthV,1,'uint32');
            obj.s(index) = V;
        end
